...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create easyride-a717
  fly apps create easyride-a717-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app easyride-a717
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app easyride-a717-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'easyride-a717-staging' does not match app name in config file 'easyride-a717'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name easyride-a717-db
  fly postgres attach --app easyride-a717 easyride-a717-db

  fly postgres create --name easyride-a717-staging-db
  fly postgres attach --app easyride-a717-staging easyride-a717-staging-db
  ```
